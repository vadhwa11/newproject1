...
function selectSNOMEDCT(state_IN, semantictag_IN, acceptability_IN,
	...
	jQuery(function($) {
		...
		/*
		 * $("#"+from).keyup( function(e) { if (e.which == 13) { var dataValue =
		 * document .getElementById(from).value; if (dataValue == '') { var
		 * txtBox = document.getElementById(from); txtBox.focus(); return; }
		 * 
		 * if (dataValue.length >= 3) { if
		 * (document.getElementById("conceptdiv")) $("#conceptdiv").remove();
		 * $("#msg3chars").hide(); $('#nosuggestion').hide(); var txtBox =
		 * document .getElementById(from); txtBox.blur();
		 * loadResultsList(stateParam, semantictagParam,acceptabilityParam,
		 * returnlimitParam, callback); } else { if
		 * (document.getElementById("conceptdiv")) $("#conceptdiv").remove(); } }
		 * });
		 */
...
jQuery(function($) {
	...
	$("#referral").change(
			function() {
				...
				if ($("#referral").val() == 1) {
					...
					var url = "http://localhost:8181/AshaServices/v0.1/opdmaster/getEmpanelledHospital";
					$
							.ajax({
								url : url,
								dataType : "json",
								...
								success : function(response) {
									...
									$("#diagnosisId > option").each(
											function() {
												...
												var html='<tr><td>'+(j++)+'</td><td><select id="referHospitalList" name="referHospitalList">';
																		
												html+=trHTML+'</select></td><td><input type="text" id="departmentValue" name="departmentValue" /></td><td><input type="text" id="diagonsisText" name="diagonsisText" value="'+diagonsisText+'"/></td><td><input type="text" id="hos" name="hos" /></td><td><input name="Button" type="button" class="buttonAdd" value="" tabindex="1" onclick="addRowForRefer();" /></td><td><input type="button" name="delete" value="" class="buttonDel" tabindex="1" onclick="removeRowReferal(this);" /></td><td><input type="hidden" id="diagonsisId" name="diagonsisId" value="'+diagonsisValue+'"/></td></tr>'
												$('#referalGrid').append(html);
	 ...
	 * $(this).val($(this).val().replace(/[^0-9\.]/g,'')); if ((event.keyCode !=
	 * 46) && ((event.keyCode != 37)) && ((event.keyCode != 39)) &&
	 * (event.keyCode != 9) && (event.keyCode != 8)&&(event.which != 46 ||
	 * $(this).val().indexOf('.') != -1) && (event.which < 48 || event.which >
	 * 57)) { event.preventDefault(); } });
	 * 
	 * $(".allownumericwithoutdecimal").on("keypress keyup blur",function
	 * (event) { $(this).val($(this).val().replace(/[^\d].+/, "")); if
	 * ((event.keyCode != 46) && ((event.keyCode != 37)) &&((event.keyCode !=
	 * 39)) && (event.keyCode != 9) && (event.keyCode != 8)&&(event.which < 48 ||
	 * event.which > 57)) { event.preventDefault(); } });
	 */
...
function getPresentTemplate(tempCode, fieldId) {
	// var
	// url='/hms/hms/opd?method=showPopUpPresentComplaint&'+csrf+'&'+csrfTokenName+'='+csrfTokenValue;
	if (fieldId == undefined)
		fieldId = '';
	var url = '/hms/hms/opd?method=showPopUpHistoryTemplate&tempCode='
			+ tempCode + "&fieldId=" + fieldId;
	popwindow(url);
}
...
function getFamilyHistoryTemplate(tempCode) {
	...
	// var
	// url='/hms/hms/opd?method=showPopUpFamilyHistory&'+csrf+'&'+csrfTokenName+'='+csrfTokenValue;
	var url = '/hms/hms/opd?method=showPopUpHistoryTemplate&tempCode='
			+ tempCode;
	popwindow(url);
}
...
function addRowTreatmentNursingCare() {
	...
	new Ajax.Autocompleter('procedureName_nursing' + iteration,
			...
			'registration?method=getProcedureForAutoComplete', {
				callback : function(element, entry) {
					return entry + '&minor_major=1' + '&procedureType='
							+ document.getElementById('nursingCategory').value;
					;
				},
	...
	/*
	 * var cellRight5 = row.insertCell(5); var e1 =
	 * document.createElement('input'); e1.type = 'checkbox';
	 * e1.name='alert'+iteration; e1.id='alert'+iteration;
	 * e1.className="radioCheck"; e1.size='5'; cellRight5.appendChild(e1);
	 */
}
...
function addRowTherapy(procedureType) {
	...
	newdiv.className = "autocomplete";
	cellRight1.appendChild(newdiv);
	new Ajax.Autocompleter('therapy_nursing' + iteration, 'ac2updates_therapy'
			+ iteration,
			'registration?method=getProcedureForAutoComplete&procedureType='
					+ procedureType, {
				callback : function(element, entry) {
	...
	/*
	 * var cellRight2 = row.insertCell(2); var e1 =
	 * document.createElement('Select'); e1.name='frequency_nursing'+iteration;
	 * e1.id='frequency_nursing'+iteration; e1.options[0] = new Option('Select',
	 * '0'); for(var i = 0;i<icdArray.length;i++ ){ e1.options[i+1] = new
	 * Option(icdArray[i][1],icdArray[i][0]); } cellRight2.appendChild(e1);
	 * 
	 * var cellRight3 = row.insertCell(3); var e1 =
	 * document.createElement('input'); e1.type = 'text';
	 * e1.name='noOfDays_nursing'+iteration; e1.className="opdTextBoxTSmall
	 * textYellow"; e1.id='noOfDays_nursing'+iteration; e1.size='5';
	 * cellRight3.appendChild(e1);
	 */
	 ...
	 * "setdate('26/07/2017',document.getElementById('appointmentDate"+iteration+"'),event)"); };
	 */
	e1.setAttribute("onclick", "clearAppointment('appointmentDate" + iteration
			+ "','appointmentTime" + iteration + "'," + iteration + ")");
	...
	e3.setAttribute("onblur", "validateExpDate(this,'appointmentDate"
			+ iteration + "')");
	...
	div3.innerHTML = "Appointment Date <span id='appointDateSpan" + iteration
			+ "' style='display: none'>*</span>";
	cellRight4.appendChild(div3);
	cellRight4.appendChild(e3);

	/*
	 * var e3 = document.createElement('input'); e3.type = "text";
	 * e3.name='appointmentDate'+iteration; e3.id='appointmentDate'+iteration;
	 * var div3 = document.createElement("div"); div3.style.width = "110px";
	 * div3.style.float = "left"; div3.innerHTML = "Appointment Date <span
	 * id='appointDateSpan"+iteration+"' style='display: none'>*</span>";
	 * cellRight4.appendChild(div3); cellRight4.appendChild(e3);
	 * 
	 * var e4 = document.createElement('img'); e4.src =
	 * '/hms/jsp/images/cal.gif'; e4.name='appointmentDate'+iteration;
	 * e4.id='calendar'; e4.width='16'; e4.height='16'; e4.onclick =
	 * function(event) {
	...
	div4.innerHTML = "Appointment Time <span id='appointTimeSpan" + iteration
			+ "' style='display: none'>*</span>";
...
function removeRowTherapy() {
	...
	for (var i = 0; i <= iteration; i++) {
		if (document.getElementById("therapyRadio" + i) != null
				&& (typeof document.getElementById("therapyRadio" + i).checked) != 'undefined'
				&& document.getElementById("therapyRadio" + i).checked) {
			totalSelected = totalSelected + 1;
		}
	}
	if (totalSelected == 0) {
		...
		for (var i = 0; i <= iteration; i++) {
			if (document.getElementById("therapyRadio" + i) != null
				...
				document.getElementById("therapyRadio" + i).parentNode.parentNode.parentNode
						.removeChild(deleteRow);
			}
		}
	}
...
function addRowTreatmentSurgery() {
	...
	cellRight1.appendChild(newdiv);
	new Ajax.Autocompleter('procedureName_surgery' + iteration,
			'ac2updates_surgery' + iteration,
 ...
 * END function addRow(){
 * 
 * var tbl = document.getElementById('grid'); var hdbTabIndex =
 * parseInt(document.getElementById('hdbTabIndex').value)+1;
 ...
 * iteration = lastRow + 1 var iteration = lastRow; var row =
 * tbl.insertRow(lastRow); var hdb = document.getElementById('hdb'); iteration =
 ...
 * e1.type = 'checkbox'; e1.name='itemRadio'+iteration;
 * e1.id='itemRadio'+iteration; e1.className='radioCheck';
 * e1.onchange=function(){ checkPrescriptionCheck(iteration); }; e1.tabIndex
 * =hdbTabIndex+"1" ; cellRight1.appendChild(e1);
 * 
 * var e1 = document.createElement('input'); e1.type = 'hidden';
 * e1.name='prescription_availableStatus'+iteration;
 * e1.id='prescription_availableStatus'+iteration; e1.className="textYellow
 * grdTextSmall"; cellRight1.appendChild(e1);
 * 
 * 
 * var cellRight1 = row.insertCell(1); var e1 = document.createElement('input');
 * e1.type = 'text'; e1.name='nomenclature'+iteration;
 * e1.id='nomenclature'+iteration; e1.className="textYellow largTextBoxOpd";
 ...
 * e1.size='35'; e1.tabIndex =hdbTabIndex+"2" ; cellRight1.appendChild(e1);
 * e1.focus();
 * 
 * var newdiv = document.createElement('div'); newdiv.setAttribute('id',
 * 'ac2updates'+iteration); newdiv.style.display = 'none'; newdiv.className =
 * "autocomplete"; cellRight1.appendChild(newdiv); new
 * Ajax.Autocompleter('nomenclature'+iteration,'ac2updates'+iteration,'opd?method=getItemListForAutoCompleteItem',{minChars:3,parameters:'requiredField=nomenclature'+iteration});
 * 
 * 
 * var e1 = document.createElement('input'); e1.type = 'hidden';
 * e1.name='brandId'+iteration; e1.id='brandId'+iteration;
 * cellRight1.appendChild(e1);
 * 
 * var e1 = document.createElement('input'); e1.type = 'hidden';
 * e1.name='manufactureId'+iteration; e1.id='manufactureId'+iteration;
 * cellRight1.appendChild(e1);
 * 
 * var e1 = document.createElement('input'); e1.type = 'hidden';
 * e1.name='pvmsNo'+iteration; e1.id='pvmsNo'+iteration;
 * cellRight1.appendChild(e1);
 * 
 * var e1 = document.createElement('input'); e1.type = 'hidden';
 * e1.name='actualDispensingQty'+iteration;
 * e1.id='actualDispensingQty'+iteration; cellRight1.appendChild(e1);
 * 
 * var cellRight1 = row.insertCell(2); var e1 = document.createElement('input');
 * e1.name='route'+iteration; e1.id='route'+iteration; e1.className="textYellow
 * opdgridText"; e1.onblur=function() { fillRouteOnTAb(iteration);};
 * cellRight1.appendChild(e1);
 * 
 * var e1 = document.createElement('input'); e1.type = 'hidden';
 ...
 * 
 * var newdiv = document.createElement('div'); newdiv.setAttribute('id',
 * 'ac2updatesRoute'+iteration); newdiv.style.display = 'none'; newdiv.className =
 * "autocomplete"; cellRight1.appendChild(newdiv); new
 ...
 * document.createElement('Select'); e1.name='route'+iteration;
 * e1.id='route'+iteration; e1.style.width = "90px";
 * e1.style.background="#FFFF99"; e1.tabIndex =hdbTabIndex+"3" ; e1.options[0] =
 * new Option('Select', '0'); for(var i = 0;i<routeArray.length;i++ ){
 ...
 * e1.name='dosage'+iteration; e1.id='dosage'+iteration;
 * e1.className="textYellow opdTextBoxTSmall"; e1.onblur=function() {
 * fillValue(this.value,iteration); }; e1.tabIndex =hdbTabIndex+"4" ;
 * cellRight1.appendChild(e1);
 * 
 * var cellRight1 = row.insertCell(4); var e1 = document.createElement('input');
 * e1.type = 'text'; e1.name='unit'+iteration; e1.id='unit'+iteration;
 * e1.className='textYellow opdTextBoxTSmall'; e1.readOnly='readOnly';
 * e1.tabIndex =hdbTabIndex+"5" ; cellRight1.appendChild(e1);
 * 
 * 
 * var cellRight1 = row.insertCell(5); var e1 =
 * document.createElement('Select'); e1.name='frequency'+iteration;
 * e1.id='frequency'+iteration; e1.style.width = "90px";
 * e1.style.background="#FFFF99"; e1.tabIndex =hdbTabIndex+"6" ; e1.options[0] =
 * new Option('Select', '0'); for(var i = 0;i<icdArray.length;i++ ){
 * e1.options[i+1] = new Option(icdArray[i][1],icdArray[i][0]); }
 * e1.onblur=function()
 * {getFrequencyValue(this.value,iteration);fillValue(this.value,iteration);displaySOSQty(this.value,iteration);};
 * cellRight1.appendChild(e1);
 * 
 * var e21 = document.createElement('input'); e21.type = 'hidden';
 * e21.name='sosQty'+iteration; e21.id='sosQty'+iteration; e21.size='5';
 * e21.setAttribute('tabindex','1'); cellRight1.appendChild(e21);
 * 
 * var e21 = document.createElement('input'); e21.type = 'hidden';
 * e21.name='frequencyValue'+iteration; e21.id='frequencyValue'+iteration;
 * e21.size='5'; e21.setAttribute('tabindex','1'); cellRight1.appendChild(e21);
 * 
 * 
 * var cellRight1 = row.insertCell(6); var e1 = document.createElement('input');
 * e1.type = 'text'; e1.name='noOfDays'+iteration; e1.className="textYellow
 * opdTextBoxTSmall"; e1.id='noOfDays'+iteration; e1.size='3'; e1.tabIndex
 * =hdbTabIndex+"7" ; e1.onblur=function() {
 * fillValueDays(iteration);fillValue(this.value,iteration); };
 * cellRight1.appendChild(e1);
 * 
 * var cellRight1 = row.insertCell(7); var e1 =
 * document.createElement('Select'); e1.name='instrunction'+iteration;
 * e1.id='instrunction'+iteration; e1.style.width = "90px";
 * e1.style.background="#FFFF99"; e1.tabIndex =hdbTabIndex+"8" ; e1.options[0] =
 * new Option('Select', '0'); for(var i = 0;i<instructionArray.length;i++ ){
 * e1.options[i+1] = new Option(instructionArray[i][1],instructionArray[i][0]); }
 * e1.onblur=function() { fillInstrunctionOnTAb(iteration); };
 * cellRight1.appendChild(e1);
 * 
 * var cellRight1 = row.insertCell(8); var e1 = document.createElement('input');
 * e1.type = 'text'; e1.name='splInstrunction'+iteration;
 * e1.id='splInstrunction'+iteration; e1.tabIndex =hdbTabIndex+"9" ;
 * e1.className="textYellow opdTextBoxSmall"; e1.onblur=function() {
 * fillSPLInstrunctionOnPTAb(iteration); }; e1.size='5';
 * cellRight1.appendChild(e1);
 * 
 * 
 * var cellRight1 = row.insertCell(9); var e1 = document.createElement('input');
 * e1.type = 'text'; e1.name='total'+iteration; e1.id='total'+iteration;
 * e1.className="textYellow opdTextBoxTSmall"; e1.readOnly='readOnly';
 * e1.size='5'; cellRight1.appendChild(e1);
 * 
 * var cellRight1 = row.insertCell(10); var e1 =
 * document.createElement('input'); e1.type = 'text';
 * e1.name='unitLable'+iteration; e1.id='unitLable'+iteration;
 * e1.className="textYellow opdTextBoxTSmall"; e1.readOnly='readOnly';
 * e1.size='5'; cellRight1.appendChild(e1); }
 */
function addRow() {
	...
	e0.onblur = function() {
		...
		if (val != "") {
			...
			checkItem(iteration);
			disableOtherMedicine(this.value, iteration);
			checkForAlreadyIssuedPrescription(this.value, iteration, visitId);
			displayAu(this.value, iteration, '<%= hinId%>');
			checkForPurchase(this.value, iteration);
			checkForContradiction(this.value, iteration);
		} else {
			document.getElementById('nomenclature' + iteration).value = "";
			document.getElementById('pvmsNo' + iteration).value = "";
	...
	// alert("3--3-"+iteration);
	// new
	// Ajax.Autocompleter('nomenclature'+iteration,'ac2update'+iteration,'opd?method=getItemListForAutoCompleteItem',{parameters:'requiredField=nomenclature'+iteration});
	// alert("name--"+e0.name)
	// alert("4---");
	/*
	 * var cellRight1 = row.insertCell(1); var eImg =
	 * document.createElement('img'); eImg.src = '/hms/jsp/images/search.gif';
	 * eImg.name = 'search' + iteration; eImg.id = 'search' + iteration;
	 * eImg.WIDTH = '26'; eImg.HEIGHT = '26'; //eImg.id =
	 * 'billDateId'+iteration; eImg.onclick = function(){ var
	 * url="/hms/hms/opd?method=showItemSearchJsp&count="+iteration;
	...
	for (var i = 0; i < itemClassArray.length; i++) {
		e12.options[i + 1] = new Option(itemClassArray[i][1],
				itemClassArray[i][0]);
	}
	...
	/*
	 * var cellRight3 = row.insertCell(4); var e31 =
	 * document.createElement('input'); e31.type = 'checkbox';
	 * e31.name='injCategory'+iteration; e31.id='injCategory'+iteration
	 * e31.size='10'; e31.className='radio'; e31.value='y';
	 * e31.setAttribute('tabindex','1'); cellRight3.appendChild(e31);
	 */
	...
	e14.onblur = function() {
		checkDosageValidation(this.value, iteration);
		fillValue(iteration)
	};
	...
	/*
	 * var cellRight6 = row.insertCell(6); var e6 =
	 * document.createElement('Select');
	 * 
	 * e6.name='instructionACPC'+iteration; e6.id='instructionACPC'+iteration;
	 * e6.classname='smalllabel'; e6.setAttribute('tabindex','1'); e6.options[0] =
	 * new Option('Select', ''); e6.options[1] = new Option('AC', 'AC');
	 * e6.options[2] = new Option('PC', 'PC',true); cellRight6.appendChild(e6);
	 * 
	 * 
	 * var cellRight7 = row.insertCell(7); var e7 =
	 * document.createElement('Select');
	 * 
	 * e7.name='typeLeftRight'+iteration; e7.id='typeLeftRight'+iteration;
	 * e7.classname='smalllabel'; e7.setAttribute('tabindex','1'); e7.options[0] =
	 * new Option('Select', ''); e7.options[1] = new Option('Left', 'left');
	 * e7.options[2] = new Option('Right', 'right'); cellRight7.appendChild(e7);
	 */
	...
	/*
	 * var cellRight10 = row.insertCell(9); var e71 =
	 * document.createElement('input'); e71.type = 'checkbox';
	 * e71.name='ct'+iteration; e71.id='ct'+iteration e71.size='10';
	 * e71.className='radio'; e71.value='y'; e71.setAttribute('tabindex','1');
...
function display(obj, val) {
	...
	if (val != 'Nicu Case Record') {
		...
		if (splName == '' || splName != val) {
			submitProtoAjaxNew('opdMain',
					'/hms/hms/opd?method=showNicuCaseRecordJsp',
					'specialityDiv');
		}
	}
}
...
function fnGetHospitalDepartment(hospitalId) {
	// new
	// Ajax.Request('opd?method=getHospitalDepartment&hospitalId='+hospitalId+'&'+csrfTokenName+'='+csrfTokenValue,
	// {
	new Ajax.Request(
...
function fnGetHospitalWards(hospitalId) {
	...
	// {
	new Ajax.Request(
			'opd?method=getHospitalWards&hospitalId=' + hospitalId,
			{
				onSuccess : function(response) {
					if (response.responseText.trim() != '') {
						document.getElementById('referdepartment').innerHTML = response.responseText
...
function fillValueDays(inc) {
	document.getElementById('noOfDayspTab' + inc).value = document
			.getElementById('noOfDays' + inc).value;
}
...
/*
 * function fillValue(value,inc,from){ var dosage ; var freq; var dispenseQty;
 * var noOfDays ; var sosQty; if(from!='tab'){ alert("tab"
 * +document.getElementById('dosage'+inc)); dosage =
 * document.getElementById('dosage'+inc).value;
 * freq=document.getElementById('frequencyValue'+inc).value; dispenseQty =
 * document.getElementById('actualDispensingQty'+inc).value; noOfDays =
 * document.getElementById('noOfDays'+inc).value; sosQty =
 * document.getElementById('sosQty'+inc).value; if(freq>0 && dosage>0 &&
 ...
 * document.getElementById('totalpTab'+inc).value=total; } }else{
 * if(document.getElementById('actualDispensingQty'+inc).value != 0){ var
 * totalQty =
 * (parseFloat(freq*sosQty*dosage)/parseFloat(dispenseQty)).toFixed(2);
 * if(totalQty != '0.00'){ finalQty = freq>0?Math.ceil(totalQty):""; }
 ...
 * }else{ dosage = document.getElementById('dosagepTab'+inc).value
...
/*
 * function displayAu(val,inc){ if(val != "") { var index1 =
 * val.lastIndexOf("["); var indexForBrandName=index1; var index2 =
 * val.lastIndexOf("]"); index1++; var pvmsNo = val.substring(index1,index2);
 * var indexForBrandName=indexForBrandName--; var
 * brandName=val.substring(0,indexForBrandName); if(pvmsNo == "") {
 * document.getElementById('nomenclature'+inc).value="";
 * document.getElementById('pvmsNo'+inc).value=""; return; } else var xmlHttp;
 * try { // Firefox, Opera 8.0+, Safari xmlHttp=new XMLHttpRequest(); }catch
 * (e){ // Internet Explorer try{ xmlHttp=new ActiveXObject("Msxml2.XMLHTTP");
 * }catch (e){ alert(e) try{ xmlHttp=new ActiveXObject("Microsoft.XMLHTTP");
 * }catch (e){ alert("Your browser does not support AJAX!"); return false; } } }
 * 
 * xmlHttp.onreadystatechange=function() { if(xmlHttp.readyState==4){ var items
 * =xmlHttp.responseXML.getElementsByTagName('items')[0]; for (loop = 0; loop <
 * items.childNodes.length; loop++) { var item = items.childNodes[loop];
 * 
 * var au = item.getElementsByTagName("au")[0]; var actualDispensingQty =
 * item.getElementsByTagName("actualDispensingQty")[0]; var stock =
 * item.getElementsByTagName("stock")[0];
 * 
 ...
 * if(document.getElementById('closingStock'+inc) && stock.childNodes[0] !=
 * undefined){ document.getElementById('closingStock'+inc).value =
 * stock.childNodes[0].nodeValue; if(stock.childNodes[0].nodeValue == 0){
 * alert("Stock is not available..."); } }else{ }
 ...
 * actualDispensingQty.childNodes[0].nodeValue; }else{
 * document.getElementById('actualDispensingQty'+inc).value = 0; } } var
 * dangerousDrug = item.getElementsByTagName("dangerousDrug")[0];
 * if(dangerousDrug.childNodes[0]!=undefined &&
 * dangerousDrug.childNodes[0].nodeValue == 'y'){ alert("This drug is
 * dangerous."); } } } } var url="/hms/hms/opd?method=displayAU&pvmsNo="+pvmsNo;
 * xmlHttp.open("GET",url,true); xmlHttp.setRequestHeader("Content-Type",
 * "text/xml"); xmlHttp.send(null); } else {
		...
		if (totalSelected == 0) {
			...
			for (var i = 1; i <= iteration; i++) {
				if (document.getElementById("chargeRadio" + i) != null
						...
						&& document.getElementById("chargeRadio" + i).checked) {
					jQuery(function($) {
						var ids = document.getElementById("parkInvestigationId"
								+ i).value;
...
function addRowForInvestigation() {
	...
	new Ajax.Autocompleter('chargeCodeName' + iteration, 'ac2updateInv'
			+ iteration, 'opd?method=getInvestigationListForAutoComplete', {
		callback : function(element, entry) {
			return entry + '&labradiologyCheck='
					+ document.getElementById('investigationCategory').value;
	...
	e3.setAttribute("onblur", "validateExpDate(this,'investigationDate"
			+ iteration + "')");
	e3.setAttribute("onkeyup", "mask(this.value,this,'2,5','/')");
...
/*
 * function populatePVMS(val,inc){ if(val != "") { var index1 =
 * val.lastIndexOf("["); var indexForBrandName=index1; var index2 =
 * val.lastIndexOf("]"); index1++; var pvmsNo = val.substring(index1,index2);
 * var indexForBrandName=indexForBrandName--; var
 * brandName=val.substring(0,indexForBrandName);
 * 
 * if(pvmsNo == "") { document.getElementById('nomenclature'+inc).value="";
 * document.getElementById('pvmsNo'+inc).value="";
 * document.getElementById('dosage'+inc).value="";
 * document.getElementById('noOfDays'+inc).value="";
 * document.getElementById('unit'+inc).value="";
 * 
 * return; } else { document.getElementById('pvmsNo'+inc).value=pvmsNo;
 * document.getElementById('dosage'+inc).value=1;
 * document.getElementById('noOfDays'+inc).value=1;
 * 
 * new
 * Ajax.Request('ipd?method=updateItemUnit&pvmsNo='+pvmsNo+'&'+csrfTokenName+'='+csrfTokenValue, {
 * onSuccess: function(response) { if(response.responseText.trim()!='') { var
 * str=response.responseText.trim().split("###");
 * document.getElementById('unit'+inc).value=str[0];
 * document.getElementById('unitpTab'+inc).value=str[0];
 * document.getElementById('unitLable'+inc).value =str[1]!=undefined?str[1]:"";
 * document.getElementById('unitLablepTab'+inc).value
 * =str[1]!=undefined?str[1]:""; } } }); } } else {
 * document.getElementById('nomenclature'+inc).value="";
 * document.getElementById('pvmsNo'+inc).value="";
 * document.getElementById('dosage'+inc).value="";
 * document.getElementById('noOfDays'+inc).value="";
...
function addNomenclatureRow() {
	...
	autocomplete(val, arryNomenclature);
	/*
	 * // var visitId = document.getElementById('visitId').value; var tbl =
	 * document.getElementById('nomenclatureGrid');
	 * 
	 * var lastRow = tbl.rows.length;
	 * 
	 * var iteration = lastRow; var row = tbl.insertRow(lastRow); var
	 * nomenclaturehdb = document.getElementById('nomenclaturehdb'); var
	 * iteration = parseInt(nomenclaturehdb.value)+1;
	 * nomenclaturehdb.value=iteration var cellRight0 = row.insertCell(0); var
	 * e0 = document.createElement('input'); e0.type = 'text';
	 * 
	 * 
	 * e0.name = 'nomenclature' + iteration; e0.id = 'nomenclature' + iteration;
	 * e0.onblur=function(){ var val=e0.value if(val != "") { var index1 =
	 * val.lastIndexOf("["); var indexForPvms=index1; var index2 =
	 * val.lastIndexOf("]"); index1++; var pvmsNo =
	 * val.substring(index1,index2); var indexForPvms=indexForPvms--; var
	 * nomenclature=val.substring(0,indexForPvms); if(pvmsNo =="") {
	 * document.getElementById('nomenclature'+iteration).value="";
	 * document.getElementById('pvmsNo'+iteration).value="";
	 * //disableOtherMedicine(this.value,iteration); return; } else
	 * document.getElementById('pvmsNo'+iteration).value=pvmsNo
	 * checkItem(iteration);checkForAlreadyIssuedPrescription(this.value,iteration,visitId);displayAu(this.value,iteration,'<%=
	 * hinId%>');checkForPurchase(this.value,iteration);checkForContradiction(this.value,iteration); }
	 * else { document.getElementById('nomenclature'+iteration).value="";
	 * document.getElementById('pvmsNo'+iteration).value="";
	 * document.getElementById('itemId'+iteration).value="";
	 * //disableOtherMedicine(this.value,iteration); return; } };
	 * 
	 * var newdiv = document.createElement('div');
	 * newdiv.id='ac2update'+iteration; newdiv.className='autocomplete';
	 * newdiv.style.display = 'none'; e0.size = '77';
	 * e0.setAttribute('tabindex','1');
	 * 
	 * var e01 = document.createElement('input'); e01.type = 'hidden'; e01.name =
	 * 'itemId' + iteration; e01.id = 'itemId' + iteration; e0.focus(); var e02 =
	 * document.createElement('input'); e02.type = 'hidden'; e02.name =
	 * 'itemIdClassificationId' + iteration; e02.id = 'itemIdClassificationId' +
	 * iteration;
	 * 
	 * cellRight0.appendChild(e0); cellRight0.appendChild(e01);
	 * cellRight0.appendChild(e02); cellRight0.appendChild(newdiv);
	 * 
	 * 
	 * var cellRight15 = row.insertCell(1); var e12 =
	 * document.createElement('input'); e12.type = 'text';
	 * e12.name='dispensingUnit'+iteration; e12.id='dispensingUnit'+iteration
	 * e12.readOnly=true; e12.size='6'; e12.setAttribute('tabindex','1');
	 * //e12.onblur=function(){displayAU(this.value,iteration)};
	 * cellRight15.appendChild(e12); // var cellRight3 = row.insertCell(4); var
	 * e1 = document.createElement('input'); e1.type = 'hidden';
	 * e1.name='actualDispensingQty'+iteration;
	 * e1.id='actualDispensingQty'+iteration e1.size='6';
	 * e1.setAttribute('tabindex','1');
	 * 
	 * 
	 * var e15 = document.createElement('input'); e15.type = 'hidden';
	 * e15.name='highValueMedicine'+iteration;
	 * e15.id='highValueMedicine'+iteration e15.size='1';
	 * e15.setAttribute('tabindex','1'); cellRight15.appendChild(e15);
	 * 
	 * var e016 = document.createElement('input'); e016.type = 'hidden';
	 * e016.name='itemClassCode'+iteration; e016.id='itemClassCode'+iteration
	 * e016.size='6'; e016.setAttribute('tabindex','1');
	 * cellRight15.appendChild(e016); //end var e13 =
	 * document.createElement('input'); e13.type = 'hidden';
	 * e13.name='au'+iteration; e13.id='au'+iteration e13.size='6';
	 * e13.setAttribute('tabindex','1');
	 * //e12.onblur=function(){displayAU(this.value,iteration)};
	 * cellRight15.appendChild(e13); cellRight15.appendChild(e1);
	 * 
	 * 
	 * var cellRight4 = row.insertCell(2); var e14 =
	 * document.createElement('input'); e14.type = 'text';
	 * e14.name='dosage'+iteration; e14.id='dosage'+iteration e14.size='5';
	 * e14.setAttribute('maxlength', 5); e14.setAttribute('tabindex','1'); //
	 * e14.onblur =
	 * function(){checkDosageValidation(this.value,iteration);fillValue(iteration)};
	 * cellRight4.appendChild(e14);
	 * 
	 * 
	 * 
	 * var sel = document.createElement('input'); sel.type = 'hidden';
	 * sel.name='pvmsNo'+iteration; sel.id='pvmsNo'+iteration sel.size = '10';
	 * sel.setAttribute('tabindex','1'); cellRight4.appendChild(sel); // var
	 * cellRightSel = row.insertCell(2);
	 * 
	 * var cellRight5 = row.insertCell(3); var e2 =
	 * document.createElement('Select'); e2.name='frequency'+iteration;
	 * e2.id='frequency'+iteration; e2.className='medium'; //e2.class =
	 * 'medium'; e2.setAttribute('tabindex','1'); e2.options[0] = new
	 * Option('Select', '0');
	 ...
	 * for(var i = 0;i<icdArray.length;i++ ){ e2.options[i+1] = new
	 * Option(icdArray[i][1],icdArray[i][0]); } cellRight5.appendChild(e2); var
	 * e52 = document.createElement('input'); e52.type = 'text';
	 * e52.name='sosQty'+iteration; e52.id='sosQty'+iteration; e52.tabIndex='1';
	 * e52.size='3'; e52.style.display='none'; e52.setAttribute('maxlength', 3);
	 * e52.onblur=function(){fillValue(iteration)}; cellRight5.appendChild(e52);
	 * 
	 * var e21 = document.createElement('input'); e21.type = 'hidden';
	 * e21.name='frequencyValue'+iteration; e21.id='frequencyValue'+iteration;
	 * e21.size='5'; e21.setAttribute('tabindex','1');
	 * cellRight5.appendChild(e21);
	 * 
	 * var cellRight6 = row.insertCell(4); var e4 =
	 * document.createElement('input'); e4.type = 'text';
	 * e4.name='noOfDays'+iteration; e4.id='noOfDays'+iteration; e4.size='5';
	 * e4.setAttribute('maxlength', 3); e4.setAttribute('tabindex','1');
	 * e4.setAttribute('validate','No. of Days,int,no');
	 * e4.onblur=function(){fillValue(iteration)}; cellRight6.appendChild(e4);
	 * 
	 * 
	 * var cellRight16 = row.insertCell(5); var e5 =
	 * document.createElement('input'); e5.type = 'text';
	 * e5.name='total'+iteration; e5.id='total'+iteration; e5.size='5';
	 * e5.setAttribute('tabindex','1'); // validate="total,num,no"
	 * e5.setAttribute('validate','total,num,no'); //
	 * e5.onblur=function(){treatmentTotalAlert(this.value,iteration)};
	 * cellRight16.appendChild(e5);
	 * 
	 * 
	 * var cellRight9 = row.insertCell(6); var e7 =
	 * document.createElement('input'); e7.type = 'text';
	 * e7.name='remarks'+iteration; e7.id='remarks'+iteration e7.size='10';
	 * e7.setAttribute('placeholder','1-1-1'); e7.setAttribute('maxlength', 15);
	 * e7.setAttribute('tabindex','1'); cellRight9.appendChild(e7);
	 * 
	 * 
	 * var cellRight11 = row.insertCell(7); var e72 =
	 * document.createElement('input'); e72.type = 'text';
	 * e72.name='closingStock'+iteration; e72.id='closingStock'+iteration
	 * e72.size='3'; e72.setAttribute('tabindex','1');
	 * cellRight11.appendChild(e72);
	 * 
	 * var cellRight12 = row.insertCell(8); var e8 =
	 * document.createElement('input'); e8.type = 'button'; e8.className =
	 * 'buttonAdd'; e8.value = ""; e8.name='remarks'+iteration; //
	 * e8.setAttribute('onClick', 'addRow();'); e8.onclick =
	 * function(){addNomenclatureRow();}; e8.setAttribute('tabindex','1');
	 * cellRight12.appendChild(e8);
	 * 
	 * var cellRight13 = row.insertCell(9); var e9 =
	 * document.createElement('input'); e9.type = 'button'; e9.className =
	 * 'buttonDel'; e9.value = ""; e9.name='remarks'+iteration;
	 * //e9.setAttribute('onClick', 'removeRow("grid","hdb",this);'); e9.onclick =
	 * function(){removeRow("nomenclatureGrid","nomenclaturehdb",this);};
	 * e9.setAttribute('tabindex','1'); cellRight13.appendChild(e9);
	 */
...
function getTemplateDetail() {
	...
	var pathname = window.location.pathname;
	var accessGroup = pathname.split("/")[1];
	var url = "http://localhost:8181/AshaServices/v0.1/opdmaster/getTemplateName";
	$
			.ajax({
				url : url,
				dataType : "json",
				...
				success : function(response) {
					...
					$('#investigationTemplateId').html(trHTML);
...
function getInpaneldHospitalList() {
	...
	var pathname = window.location.pathname;
	var accessGroup = pathname.split("/")[1];
	var url = "http://localhost:8181/AshaServices/v0.1/opdmaster/getMasFrequency";
	$
			.ajax({
				url : url,
				dataType : "json",
...
function addRowForNIP() {
	...
	// alert("3--3-"+iteration);
	new Ajax.Autocompleter('nomenclature' + iteration, 'ac2update' + iteration,
			'opd?method=getNipItemListForAutoCompleteItem', {
	...
	e11.onblur = function() {
		if (this.value != '') {
			checkDuplicateOtherMedicine(this.value, iteration);
			readOnlyNomenclature(this.value, iteration);
			showSimilarMedicineNames(this.value)
		} else {
			readOnlyNomenclature(this.value, iteration);
		}
	};
	...
	e5.setAttribute('validate', 'total,num,no');
	e5.onblur = function() {
		treatmentTotalAlert(this.value, iteration)
	};
	cellRight16.appendChild(e5);
	...
	// var cellRight7 = row.insertCell(7);
	/*
	 * var cellRight8 = row.insertCell(9); var e6 =
	 * document.createElement('input'); e6.type = 'text';
	 * e6.name='route'+iteration; e6.id='route'+iteration e6.size='5';
	 * e6.value='' e6.setAttribute('maxlength', 20);
	 * e6.setAttribute('tabindex','1'); cellRight8.appendChild(e6);
	 */
...
function showCreateInvestigationTemplate() {
	...
	newwindow = window.open(url, 'investigation',
			"height=500,width=1010,status=1,top=0,left=2");
}
