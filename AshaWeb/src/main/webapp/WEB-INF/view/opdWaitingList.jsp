...
<!---header text ends--->
 ...
 <div class="hName"><h4>OPD CONSULTATION</h4></div>
 ...
 $(document).ready(
		function() {
			...
			$.ajax({
				...
				success : function(res) {
					...
					html += '<tr ><td style=display:none; >'
								...
								+ value.patinetname + '</td><td>'
								+ value.relation+ '</td><td>'
								+ value.rankName + '</td><td>'
								+ value.empName + '</td><td>'
								+ value.age + '</td><td>'
								+ value.gender + '</td><td>'
								+ value.mobileNumber + '</td><td>'
								+ "<input type='submit' value='Release' id='released' />" + '</td><td>'
								+"<input type='submit' value='Close' id='closed' />" + '</td></tr >'
			...
			.on(
					...
					'tr',
					function() {
						var visitId =   $(this).closest('tr').find('td:eq(0)').text();
						 //alert("localStorage clear patientId: "+ patientId);
						 localStorage.visitId=visitId;
						 var patientId =  $(this).closest('tr').find('td:eq(1)').text();
						...
						//window.location.href = "opdMain.jsp"
