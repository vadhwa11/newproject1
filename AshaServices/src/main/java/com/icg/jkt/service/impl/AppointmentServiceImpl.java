...
public class AppointmentServiceImpl implements AppointmentService{
	...
	public Map<String, Object> getlocationWiseAppointmentType(Map<String, String> requestData) {
		...
		List<MasAppointmentSession> appointmentTypeList = appointmentDAO.getlocationWiseAppointmentType(departmentId);
	...
	public String submitAppointmentSetup(String requestData) {
		...
		for(int i=0;i<appointment.length();i++) {
			...
			else
			{
				...
				List<MasAppointmentSession> appointmentSessionsList =  appointmentDAO.validateAppointmentSetup(appSetup);
				if(appointmentSessionsList!=null && appointmentSessionsList.size()>0) {
					if(appointmentSessionsList.get(0).getMasDepartment().getDepartmentName().equalsIgnoreCase(appSetup.getMasDepartment().getDepartmentName())) {
						json.put("status", 2);
						json.put("msg", "Department Name Already Existing");
					}
					if(appointmentSessionsList.get(0).getMasHospital().getHospitalName().equalsIgnoreCase(appSetup.getMasHospital().getHospitalName())) {
						json.put("status", 2);
						json.put("msg", "Hospital Name Already Existing");
					}
					if(appointmentSessionsList.get(0).getMasAppointmentType().getAppointmentTypeName().equalsIgnoreCase(appSetup.getMasAppointmentSession().getMasAppointmentType().getAppointmentTypeName())) {
						json.put("status", 2);
						json.put("msg", "Appointment Type Already Existing");
					}
				}else {
	...
	public String getAllAppointmentSession(JSONObject jsonObject) {
		...
		if (appointmentSessionMap.get("masAppointSessionList") != null) {
			...
			for (MasAppointmentSession masAppointmentSession : appointmentSessionsList) {
				...
				mapObj.put("hospitalName", masAppointmentSession.getMasHospital().getHospitalName());
